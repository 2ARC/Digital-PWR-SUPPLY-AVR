...
//#define SWVERSION "ver: ddcp-0.5.3"
...
static void store_permanent(void){
	...
	if (changeflag){
		...
		eeprom_write_word((uint16_t *)0x04,set_val[1]);
			...
			lcd_puts_P("JA FOI SALVO"); // alterado 13/05/2016
	}
	_delay_ms(200);
}
...
int main(void)
{
	...
	lcd_puts_P("marcoadeoli@");
	lcd_gotoxy(0,1);
	lcd_puts_P("outlook.com");
	_delay_ms(1000);
	lcd_clrscr();
	lcd_puts_P("Vmax = 30V");
	lcd_gotoxy(0,1);
	...
	lcd_clrscr();
	init_kbd();
	DDRC |= (1 << PINC4); // Linha RW do LCD, permite MCU rodar a 8MHz
	set_val[0]=30;set_val[1]=50; // 300mA and 5V
	if (eeprom_read_byte((uint8_t *)0x0) == 19){
		// ok magic number matches accept values
	...
	while (1) {
		...
		if (i==30){ // not every round to avoid flicker
			//lcd_reset();
			lcd_clrscr();
			i=0;
		}
		...
		if (check_buttons()==0){
			...
			// button press
			// número de saltos antes de acelerar
			if (bpress > 11){
